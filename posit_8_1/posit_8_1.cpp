...
Posit_8_1::Scalar is_inf(Posit_8_1 num){
    return (num.m_posit == 0x80);
}
Posit_8_1::Posit_8_1():m_size(8){}


Posit_8_1::Posit_8_1(const Posit_8_1& other):m_size(8){
...
Posit_8_1::Posit_8_1(const int other, const int sig_bit, const int reg_bit, const int expo_bit, const int frac_bit):m_size(8){
    this->m_posit = other;
...
Posit_8_1& Posit_8_1::operator=(const Posit_8_1& other){
    if (this != &other) {
        m_size = other.m_size;
        this->m_posit = other.m_posit;
        this->sign_bit = other.sign_bit;
...
Posit_8_1::Scalar Posit_8_1::at(Posit_8_1::Index i){
    if ((int)i < 0 || i >= m_size) {
        throw std::out_of_range("Index out of range");
    }
    return std::bitset<8>(this->m_posit)[i];
}

Posit_8_1::Index Posit_8_1::getsize(){
    return m_size;
}

Posit_8_1::Scalar Posit_8_1::Posit_8_1_Get_Sign(){
    return sign_bit;
}

float Posit_8_1::translate_posit_8_1_to_float(){
    if((this->m_posit&0xff) == 0){
        return (float)0;
    }
    Posit_8_1::Scalar m_sign = this->Posit_8_1_Get_Sign();      
    ...
    cout << "frac = " << fraction_bit << endl;
    cout << "expo = " << exponent_bit << endl;
    cout << "reg = " << regime_bit << endl;
    if(m_sign == 1){/*negative*/
        current_posit_num = ((~current_posit_num) + 1);
    }
    current_posit_num &= 0x7f;
    //cout<<"erjinzhi=="<<std::bitset<15>(current_posit_num)<<endl;
    std::bitset<7> binary_arr(current_posit_num);
...
Posit_8_1 translate_float_to_posit_8_1(const float num){
    if(num == 0){
        return zero;
    }
 ...
 //   std::cout <<"EXPONENT===" <<bitset<32>((ori_exponent >> 3)) << std::endl;
   ...
   // printf("m ==%d\n",m);
    ...
    int remain_bit = 7 - m - regime_bit_with;
     //   printf("remian_bit ==%d\n",remain_bit);
    if(remain_bit >= 1){
        exponent_bit = 1;
...
Posit_8_1 Posit_8_1::operator+(const Posit_8_1& other) const{
    Posit_8_1 ltmp = *this;
    Posit_8_1 rtmp = other;
    if(is_zero(ltmp) || is_zero(rtmp)){
        return zero;
    }
    float num1 = ltmp.translate_posit_8_1_to_float();
    float num2 = rtmp.translate_posit_8_1_to_float();
    float num3 = num1 + num2;
    return translate_float_to_posit_8_1(num3);
}
Posit_8_1 Posit_8_1::operator+=(const Posit_8_1& other) {
    Posit_8_1 tmp = *this + other;
    *this = tmp;
    return *this;
}
Posit_8_1 Posit_8_1::operator-(const Posit_8_1& other) const{
...
bool Posit_8_1::operator<(const Posit_8_1& other) const{
    Posit_8_1 ltmp = *this;
    Posit_8_1 rtmp = other;
    float left = ltmp.translate_posit_8_1_to_float();
    float right = rtmp.translate_posit_8_1_to_float();
    return left < right;
}
bool Posit_8_1::operator<=(const Posit_8_1& other) const{
    Posit_8_1 ltmp = *this;
...
Posit_8_1& Posit_8_1::operator++(){
    *this += one;
    return *this;
}
Posit_8_1 Posit_8_1::operator++(int){
    Posit_8_1 tmp(*this);
    *this += one;
    return tmp;
}
...
int main(){
    ...
    if (file.is_open()) {
        ...
        while((unsigned int)p.m_posit <= 0xffff){
            ...
            cout << " " << endl;
            cout << p.translate_posit_8_1_to_float() << endl;
            m_int = static_cast<int>(std::floor(p.translate_posit_8_1_to_float()));
            cout << m_int << endl;
            num[m_int + 10000] += 1;
            p.m_posit++;
            for(int j = 0;j < 16; j++){
